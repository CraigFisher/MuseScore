            ...
            for (int track = startTrack; track < endTrack; ++track) {
                  ...
                  for (Chord* chord1 : chord->graceNotes()) {
                        for (Note* note1 : chord1->notes()) {
                              ...
                              //
                              int tpc  = note1->tpc();
                              int line = absStep(tpc, note1->epitch(), staff->altNoteMappings()); //cc
...
AccidentalVal Measure::findAccidental(Segment* s, int staffIdx, int line) const
      ...
      for (Segment* segment = first(st); segment; segment = segment->next(st)) {
            if (segment == s) {
                  ...
                  int l = relStep(line, clef, staff->altNoteMappings()); //cc
                  ...
                  for (Chord* chord1 : chord->graceNotes()) {
                        for (Note* note : chord1->notes()) {
                              if (note->tieBack())
                                    continue;
                              int tpc  = note->tpc();
                              int l    = absStep(tpc, note->epitch(), staff->altNoteMappings()); //cc
